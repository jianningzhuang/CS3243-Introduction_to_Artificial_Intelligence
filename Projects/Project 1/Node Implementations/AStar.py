...
class Node:
    def __init__(self, state, parent, action_cost, goals):
        self.state = state
        self.parent = parent
        if parent != None:
            self.g_cost = parent.g_cost + action_cost
        else:
            self.g_cost = 0
        self.h_cost = self.euclidean_distance(goals)
    ...
    def get_path(self):
        ...
        return (moves, self.f_cost)

    def __lt__(self, other):
        return self.f_cost < other.f_cost

    def chebyshev_distance(self, goals):
        min_dist = None
        for goal in goals:
            estimate = max(abs(self.state[0] - goal[0]), abs(self.state[1] - goal[1]))
    ...
    def euclidean_distance(self, goals):
        min_dist = None
        for goal in goals:
            estimate = math.sqrt(abs(self.state[0] - goal[0])**2 + abs(self.state[1] - goal[1])**2)/1.4143
            if min_dist == None or estimate < min_dist:
                min_dist = estimate
        return min_dist
...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    ...
    initial_node = Node(my_king, None, 0, goals)
    ...
    while len(frontier) > 0:
        ...
        for valid_move in board.actions(current.state):
            ...
            successor = Node(successor_state, current, board.grid[successor_state[0]][successor_state[1]], goals)
            if successor.state not in reached or successor.f_cost < reached[successor_state].f_cost:
