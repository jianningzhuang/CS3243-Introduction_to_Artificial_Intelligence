...
class Board:
    ...
    def set_board(self):
        ...
        for enemy in self.enemy_pieces:
            self.grid[enemy[1][0]][enemy[1][0]] = -1
        for enemy in self.enemy_pieces:
...
def get_path(current, parents):
    ...
    while parents[current] != (-1, -1):
        ...
        moves.insert(0, [(alphabet[parent[1]], parent[0]), (alphabet[current[1]], current[0])])
        current = parent
    return moves

def chebyshev_distance(current, goals):
    min_dist = None
    for goal in goals:
        estimate = max(abs(current[0] - goal[0]), abs(current[1] - goal[1]))
        if min_dist == None or estimate < min_dist:
            min_dist = estimate
    return min_dist

def euclidean_distance(current, goals):
    min_dist = None
    for goal in goals:
        estimate = math.sqrt(abs(current[0] - goal[0])**2 + abs(current[1] - goal[1])**2)/1.4143
        if min_dist == None or estimate < min_dist:
            min_dist = estimate
    return min_dist

def manhatten_distance(current, goals):
    min_dist = None
    for goal in goals:
        estimate = (abs(current[0] - goal[0]) + abs(current[1] - goal[1]))/2
        if min_dist == None or estimate < min_dist:
            min_dist = estimate
    return min_dist
...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    ...
    initial_f_cost = 0 + chebyshev_distance(my_king, goals)
    heapq.heappush(frontier, (initial_f_cost, 0, my_king))
    reached[my_king] = initial_f_cost
    parents[my_king] = (-1, -1)

    while len(frontier) > 0:
        current_f_cost, current_g_cost, current = heapq.heappop(frontier)
        if current in goals:
            return (get_path(current, parents), current_f_cost)
        for valid_move in board.actions(current):
            successor = board.move_king(current, valid_move)
            successor_g_cost = current_g_cost + board.grid[successor[0]][successor[1]]
            successor_f_cost = successor_g_cost + chebyshev_distance(successor, goals)
            if successor not in reached or successor_f_cost < reached[successor]:
                heapq.heappush(frontier, (successor_f_cost, successor_g_cost, successor))
                reached[successor] = successor_f_cost
