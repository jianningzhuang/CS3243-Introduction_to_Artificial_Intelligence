...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    ...
    heapq.heappush(frontier, (0, my_king))
    reached[my_king] = 0
    parents[my_king] = (-1, -1)

    while len(frontier) > 0:
        current_path_cost, current = heapq.heappop(frontier)
        if current in goals:
            return (get_path(current, parents), current_path_cost)
        for valid_move in board.actions(current):
            successor = board.move_king(current, valid_move)
            successor_path_cost = current_path_cost + board.grid[successor[0]][successor[1]]
            if successor not in reached or successor_path_cost < reached[successor]:
                heapq.heappush(frontier, (successor_path_cost, successor))
                reached[successor] = successor_path_cost
